...
<div class="container-fluid">
    <table class="table table-form border table-striped table-hover">
        ...
        <?php foreach ($show_HistoryForAdmin as $show_HistoryForAdmins) {
            $i++ ?>
            <tr>
                <td class="border text-center mit-v"><?php echo $i ?>.</td>
                <td class="border text-center mit-v" style="width: 100px;">
                    <?php if ($show_HistoryForAdmins->trainining_request_status == "Draft" && $show_HistoryForAdmins->status == 1) { ?>
                        <a href="<?php echo base_url('index.php/ttc_controller/formRequest/?id=') ?><?php echo $show_HistoryForAdmins->id; ?>"><?php echo $show_HistoryForAdmins->tr_no; ?></a>
                    <?php } else if ($show_HistoryForAdmins->trainining_request_status == "Draft" && $show_HistoryForAdmins->status == 0) { ?>
                        <a href="<?php echo base_url('index.php/ttc_controller/formRequestStatic/') ?><?php echo $show_HistoryForAdmins->id; ?>"><?php echo $show_HistoryForAdmins->tr_no; ?></a>
                    <?php } else if (($show_HistoryForAdmins->workflow_remark == "Rejected by Admin" || $show_HistoryForAdmins->workflow_remark == "Rejected by Reviewer" || $show_HistoryForAdmins->workflow_remark == "Rejected by Approver") && $show_HistoryForAdmins->status == 1 && ($_SESSION["username"] == $show_HistoryForAdmins->submit_name)) { ?>
                        <a href="<?php echo base_url('index.php/ttc_controller/formRequestForReject/') ?><?php echo $show_HistoryForAdmins->id; ?>"><?php echo $show_HistoryForAdmins->tr_no; ?></a>
                    <?php } else if (($show_HistoryForAdmins->workflow_remark == "Requested More Information by Admin" || $show_HistoryForAdmins->workflow_remark == "Requested More Information by Reviewer" || $show_HistoryForAdmins->workflow_remark == "Requested More Information by Approver") && $show_HistoryForAdmins->status == 1 && ($_SESSION["username"] == $show_HistoryForAdmins->submit_name)) { ?>
                        <a href="<?php echo base_url('index.php/ttc_controller/formRequestForMoreInformation/') ?><?php echo $show_HistoryForAdmins->id; ?>"><?php echo $show_HistoryForAdmins->tr_no; ?></a>
                    <?php } else { ?>
                        <a href="<?php echo base_url('index.php/ttc_controller/formRequestStatic/') ?><?php echo $show_HistoryForAdmins->id; ?>"><?php echo $show_HistoryForAdmins->tr_no; ?></a>
                    <?php } ?>
                </td>
                <td class="border text-center mit-v"><?php echo $show_HistoryForAdmins->date_request; ?></td>
                <td class="border text-center mit-v" style="width: 200px;"><?php echo $show_HistoryForAdmins->requestor_name; ?></td>
                <td class="border mit-v"><?php echo $show_HistoryForAdmins->category_name; ?></td>
                <td class="border"><?php echo $show_HistoryForAdmins->course_title; ?></td>
                <td class="border text-center mit-v"><?php echo $show_HistoryForAdmins->trainining_request_status; ?></td>
                <td class="border text-center mit-v"><?php echo $show_HistoryForAdmins->workflow_remark; ?></td>
                <td class="border text-center mit-v">
                    <?php if ($show_HistoryForAdmins->status == 1) {
                ...
                <td class="border mit">
                    <?php if ($show_HistoryForAdmins->status == 1) { ?>
                        <a title="Click to Cancel" href="<?php echo base_url('index.php/ttc_controller/Cancel_Training_Request/') ?><?php echo $show_HistoryForAdmins->id; ?>" class=""><img src="<?php echo base_url(); ?>img/delete.png" alt="" style="width: 30px;"></a>
                    <?php } else {
                        echo "";
                    }; ?>
                </td>
