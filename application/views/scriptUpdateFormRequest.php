<script type="text/javascript">
    $(document).ready(function() {
                            ...
                            $.ajax({
                                ...
                                success: function(data) {
                                    $.each(data, function(index, workflow_matrix) {
                                        ...
                                        $('#up_div_flow_reviewer').append('<input type="radio" name="up_flow_reviewer[]" id="up_flow_reviewer" value="' + workflow_matrix.reviewer + ',' + workflow_matrix.reviewer_email + '" ' + (workflow_matrix.reviewer == current_reviewer ? 'checked' : '') + '>' + ' ' + '<label for="up_flow_reviewer" style="font-size: 11px;">' + workflow_matrix.reviewer + '</label>' + '<br>');
                                        if (workflow_matrix.reviewer_alter != '' && workflow_matrix.reviewer_alter != null) {
                                            $('#up_div_flow_reviewer').append('<input type="radio" name="up_flow_reviewer[]" id="up_flow_reviewer_alter" value="' + workflow_matrix.reviewer_alter + ',' + workflow_matrix.reviewer_email_alter + '" ' + (workflow_matrix.reviewer_alter == current_reviewer ? 'checked' : '') + '>' + ' ' + '<label for="up_flow_reviewer_alter" style="font-size: 11px;">' + workflow_matrix.reviewer_alter + ' ' + '(alternative)' + '</label>')
                                        }
                                        $('#up_div_flow_approver').empty();
                                        $('#up_div_flow_approver').append('<input type="radio" name="up_flow_approver[]" id="up_flow_approver" checked value="' + workflow_matrix.approver + ',' + workflow_matrix.approver_email + '" ' + (workflow_matrix.approver == current_approve ? 'checked' : '') + '>' + ' ' + '<label for="up_flow_approver" style="font-size: 11px;">' + workflow_matrix.approver + '</label>' + '<br>')
                                        if (workflow_matrix.approver_alter != '' && workflow_matrix.approver_alter != null) {
                                            $('#up_div_flow_approver').append('<input type="radio" name="up_flow_approver[]" id="up_flow_approver_alter" value="' + workflow_matrix.approver_alter + ',' + workflow_matrix.approver_email_alter + '" ' + (workflow_matrix.approver_alter == current_approve ? 'checked' : '') + '>' + ' ' + '<label for="up_flow_approver_alter" style="font-size: 11px;">' + workflow_matrix.approver_alter + ' ' + '(alternative)' + '</label>')
                                        }
                                    });
                                }
                            });
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $.each(data, function(index, workflow_matrix) {
                            ...
                            $('#up_tr_work_flow').show();
                            $('#up_div_flow_reviewer').empty();
                            $('#up_div_flow_reviewer').append('<input type="radio" name="up_flow_reviewer[]" id="up_flow_reviewer" checked value="' + workflow_matrix.reviewer + ',' + workflow_matrix.reviewer_email + '">' + ' ' + '<label for="up_flow_reviewer" style="font-size: 11px;">' + workflow_matrix.reviewer + '</label>' + '<br>')
                            if (workflow_matrix.reviewer_alter != '' && workflow_matrix.reviewer_alter != null) {
                                $('#up_div_flow_reviewer').append('<input type="radio" name="up_flow_reviewer[]" id="up_flow_reviewer_alter" value="' + workflow_matrix.reviewer_alter + ',' + workflow_matrix.reviewer_email_alter + '">' + ' ' + '<label for="up_flow_reviewer_alter" style="font-size: 11px;">' + workflow_matrix.reviewer_alter + ' ' + '(alternative)' + '</label>')
                            }
                            $('#up_div_flow_approver').empty();
                            $('#up_div_flow_approver').append('<input type="radio" name="up_flow_approver[]" id="up_flow_approver" checked value="' + workflow_matrix.approver + ',' + workflow_matrix.approver_email + '">' + ' ' + '<label for="up_flow_approver" style="font-size: 11px;">' + workflow_matrix.approver + '</label>' + '<br>')
                            if (workflow_matrix.approver_alter != '' && workflow_matrix.approver_alter != null) {
                                $('#up_div_flow_approver').append('<input type="radio" name="up_flow_approver[]" id="up_flow_approver_alter" value="' + workflow_matrix.approver_alter + ',' + workflow_matrix.approver_email_alter + '">' + ' ' + '<label for="up_flow_approver_alter" style="font-size: 11px;">' + workflow_matrix.approver_alter + ' ' + '(alternative)' + '</label>')
                            }
                        });
                    }
                });
        ...
        var up_see_attachment = $('#up_see_attachment');
        var up_file_see_attachment = $('#up_file_see_attachment');
        var view_file = $('#view_file_link');
        up_see_attachment.on("change", function() {
            if (up_see_attachment.prop("checked")) {
                up_file_see_attachment.css("display", "block");
                view_file.css("display", "block");
            } else {
                up_file_see_attachment.css("display", "none");
                view_file.css("display", "none");
                up_file_see_attachment.val("");
            }
        })
        ...
        $("[name='up_bt_submit']").click(function() {
            ...
            if (section_code.length <= 0) {
                ...
                return false;
            } else if (up_see_attachment != "" && up_file_see_attachment == "" && view_file == undefined) {
                Swal.fire({
        ...
        $("[name='up_bt_save']").click(function() {
            // return false;
            Swal.fire({
                title: 'Are you sure?',
                ...
                if (result.isConfirmed) {
                    // return true;
                    up_send_save_Training_Request_Form();
    ...
    function up_send_save_Training_Request_Form() {
        ...
        $.ajax({
            url: '<?php echo base_url(); ?>index.php/ttc_controller/up_submit_Training_Request_ajax',
            type: 'POST',
            dataType: 'json',
            data: {
                up_id: up_id,
            ...
            success: function(data) {
                Swal.fire({
                    title: "Update Save Draft successfully",
                    text: "",
                    icon: "success",
                    timer: 1200,
                    ...
                    window.location = '<?php echo base_url('index.php/ttc_controller/formRequest/?id='); ?>' + up_id;
                });
            },
            error: function(data) {
                Swal.fire({
    ...
    function up_send_submit_Training_Request_Form() {
        var up_id = document.getElementById("up_id").value;
        var online_location_date = $('#up_online_location_date').val();
        var onsite_location_date = $('#up_onsite_location_date').val();
        var email_submit = document.getElementById("up_email_submit").value;
        var submit_name = document.getElementById("up_submit_name").value;
        var year_submit = document.getElementById("up_year_submit").value;
        var section_code_list = document.getElementById("up_section_code");
        ...
        $.ajax({
            ...
            data: {
                up_id: up_id,
                online_location_date: online_location_date,
                onsite_location_date: onsite_location_date,
                email_submit: email_submit,
                submit_name: submit_name,
                year_submit: year_submit,
                section_code: section_code,
            ...
            success: function(data) {
                Swal.fire({
                    ...
                    window.location = '<?php echo base_url('index.php/email_alert/emailNewSubmit/'); ?>' + up_group_cat_admin + '/' + requestor_name + '/' + requestor_email.replace("@", "00") + '';
                });
            }
        });
    }
